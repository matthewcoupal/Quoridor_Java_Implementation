...
/**
 * Custom Linked-List for creating a graph of SpaceNodes.
 * 
...
public class SpaceLinkedList {
  ...
  /**
   * @deprecated Used only when linked list is not shaped like a grid.
   * @param x The x coordinate to locate.
   * @param y The y coordinate to locate.
   * @return The SpaceNode corresponding to both the specified x and y coordinates.
   */
  public SpaceNode spaceAtNotGrid(int x, int y) {
    SpaceNode current = front;
  ...
  /**
   * Traverses through the linked list, even with multiple node references, to find the space
  ...
  /**
   * Traverses through the a linked list, even with multiple node references, to find the space
   * matching the x and y coordinates. Note that if this is used in a default linked list, the
   * search cannot see the nodes behind its starting point.
   * 
   * @param x The x coordinate to find
   * @param y The y coordinate to find
  ...
  public SpaceNode spaceAt(int x, int y, SpaceNode startingNode) {
    ...
    if (startingNode != front) {
      startingNode =
          this.spaceAt(startingNode.getCoordinates().getX(), startingNode.getCoordinates().getY());
    }
