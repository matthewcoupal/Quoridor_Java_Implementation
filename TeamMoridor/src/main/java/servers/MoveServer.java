...
/**
 * This server starts with default name, machine name and port needs to handle MOVE -Message
 * returned by the move-server to a PLAYERS message. Indicates that it is ready to play.
 *
 * GO <move-string>- Message returned by a move-server after it is queried for a move. The
 * move-string is encoded as described in coordinates above. needs to accept Victor- prints winner
 * to screan then exits
 *
 * BOOT <player-id> - Message sent by game-client to all move-servers informing them that
 * <player-id> made an illegal move and is no longer in the game. The pawn for <player-id> should be
 * removed from the game board and any remaining walls are lost. Note: sent as the very last message
 * to the offending move-server.
 *
 *
 */

/**
 * A simple, single-threaded echo server with sequential servicing of clients. That is, when a
 * client connects, it is serviced to completion before another client connection can be accepted.
 * The server cannot be stopped gracefully: use Ctrl-C to break the running program in a terminal.
 *
 * "Echo server" means that it listens for a connection (on a user-specifiable port), reads in the
 * input on the wire, modifies the information, and writes the modified information back to the
...
public class MoveServer {
  ...
  /**
   * Creates a new <code>TCPServer</code> instance. TCPServer is a listening echo server (it
   * responds with a slightly modified version of the same message it was sent).
   *
   * @param portNumber required port number where the server will listen.
   */
  ...
  /**
   * Processes the command-line parameters and then create and run the TCPServer object.
   *
  ...
  /**
   * Primary method of the server: Opens a listening socket on the given port number (specified when
   * the object was constructed). It then loops forever, accepting connections from clients.
   *
   * When a client connects, it is assumed to be sending messages, one per line. The server will
   * process
   */
  public void run() {
  ...
  /**
   * Adds players to the list of opponents and assigns the moveServer a player number.
   * 
   * @param clientMessage String of various players.
   * @param cout the Output Stream for the MoveServer to the GameClient.
   */
  ...
  /**
   * Grabs a move from the MoveServer.
   * 
   * @param cin The input stream from the GameClient to the MoveServer.
   * @return A formatted move.
   */
  ...
  /**
   * Boots the player from the MoveServer's list of opponents and updates its logical and (if
   * needed) graphical board.
   * 
   * @param clientMessage String containing the player being booted.
   * @param sout Output Stream from MoveServer to GameClient.
   * @param sin Input Stream from GameClient to MoveServer.
   */
  ...
  public void aPlayerWent(String moveString) {
    ...
    if (move.length == 1) {
      ...
      /*
       * int i; String moveInfo[] = moveString.split(" "); for(i=0; i< opponents.length;i++){
       * if(moveInfo[1].equals(opponents[i])) break; } String cords = moveInfo[2]; Space
       * potentialPosition = board.StringtoCoordinates(cords); board.setCurrentPlayer((i));
       * board.makeMove(board.currentPlayer(),potentialPosition); if(IS_HUMAN){
       * System.out.println("updating player" + i); gui.setCurrentPlayer(i);
       * gui.makeMove(gui.currentPlayer(),potentialPosition); gui.updatePositions(); }
       */
    }
    gui.updatePositions();
  }
