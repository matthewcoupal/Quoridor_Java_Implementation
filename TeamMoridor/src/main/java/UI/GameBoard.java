...
public class GameBoard extends Board {
  ...
  public GameBoard(int numPlayers, String player) {
    ...
    panel.setPreferredSize(new Dimension(
        (((BUTTON_SIZE * BUTTON_SCALE) * BOARD_SIZE) + (BUTTON_SIZE * (BOARD_SIZE - 1))),
        (((BUTTON_SIZE * BUTTON_SCALE) * BOARD_SIZE) + (BUTTON_SIZE * (BOARD_SIZE - 1)))));
  ...
  public void addRow_PlayerB_WallBvert(int index) {
    ...
    for (i = 1; i <= BOARD_SIZE - 1; i++) {
      vertWalls[i - 1][index] =
          new WallButton_Vertical(BUTTON_SIZE, BUTTON_SCALE, i - 1, index, wallPlacement(i - 1,
              index));
  ...
  public void addRow_WallBHorizont_IntersectionB(int index) {
    int i = 0;
    horzWalls[i][index - 1] =
        new WallButton_Horizontal(BUTTON_SIZE, BUTTON_SCALE, index - 1, i, wallPlacement(i,
            index - 1));
  ...
  /**
   * Checks the occupied spaces on the board and sets the background of the space occupied by the
   * current player to RED and sets the text on all occupied spaces
   */
  ...
  /**
   * Resets the spaces' background color to the default: LIGHT_GRAY and sets all buttons' text to be
   * the empty string
   */
  public void update() {
    for (int i = 0; i < 9; i++) {
...
class WallButton_Horizontal extends JButton {
  ...
  public WallButton_Horizontal(int BUTTON_SIZE, int BUTTON_SCALE, int x, int y,
