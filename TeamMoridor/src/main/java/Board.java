...
/**
 * Is used to keep track of the positioning of players as well as check whether
 * a new configuration of the board (a player move or a wall placement) is an
 * illegal move.
 * 
...
public class Board implements BoardInterface, RulesInterface, MasterInterface {
	...
	/**
	 * Creates a new listing of players on a board based on the number of
	 * players
	 * 
	 * @param numberOfPlayers
	 *            The total number of players using this board.
	 */
	 ...
	 * 
	 * @param size
	 *            The length and width of the grid requested.
	 */
	...
	/**
	 * Checks to see whether the space being moved to is outside the scope/range
	 * of the board.
	 * 
	 * @param potentialPosition
	 *            The position the player wants to move to.
	 * 
	 * @return true if the space is out of bounds (one of the coordinates is
	 *         less than 0 or greater than 9), and is false otherwise.
	 */
	public boolean isOutOfBounds(Space potentialPosition) {
		if (potentialPosition.getX() < 0 || potentialPosition.getX() > 9
				&& potentialPosition.getY() < 0 || potentialPosition.getY() > 9) {
			return true;
		}
		return false;
	}
	 ...
	 *            The position the player wants to move to.
	 * 
	 * @return True if the jump is diagonal and a player on the space needed to
	 *         jump to, and is false otherwise.
	 */
	public boolean isMoveLegalDiagonal(Space currentPosition,
		 ...
		 * potentialPosition.getX() - currentPosition.getX());
		 * if(Math.abs(slope) == 1) { if(slope == 1) {
		 * if(potentialPosition.getX() < currentPosition.getX()) { return
		 * isPlayerHere(new Space(potentialPosition.getX() + 1,
		 * potentialPosition.getY())); } else { return isPlayerHere(new
		 * Space(potentialPosition.getX() - 1, potentialPosition.getY())); } }
		 * else if(slope == -1){ if(potentialPosition.getX() <
		 * currentPosition.getX()) { return (isPlayerHere(new
		 * Space(potentialPosition.getX() + 1, potentialPosition.getY())) ||
		 * isPlayerHere(new Space(potentialPosition.getX(),
		 * potentialPosition.getY() + 1))); } else { return isPlayerHere(new
		 * Space(potentialPosition.getX() + 1, potentialPosition.getY())); } } }
		 * }
		 */
		// return false;
	 ...
	 * 
	 * @param potentialPosition
	 *            The space the current player wants to move to.
	 * @return True if a player is located on the space; False if there is no
	 *         player located at that space.
	 */
	...
	/**
	 * Passes the grid. (Feel like this should be looked at closer. The
	 * reference will be passed, therefore I feel like anyone can edit the
	 * board.)
	 * 
	 ...
	 * 
	 * @param player
	 *            The player to check for a winning path
	 * @return True if at least one winning space has an open path to it; False
	 *         if no paths can be found;
	 */
	 ...
	 * @return True if the move to the specified space from the current space is
	 ...
	 * 
	 * @param player
	 *            The player whose position is going to be tested
	 * @return True if a specified player is located at a winning space; False
	 *         otherwise.
	 */
	 ...
	 * 
	 * @param player
	 *            The player that wants to make the jump.
	 * @param space
	 *            The space the player wants to move to.
	 * @return True if the move space is exactly 2 in one direction and another
	 ...
	 *             When a player makes a legal move, but the player is not found
	 ...
	 * 
	 * @param s0
	 *            the first space specifying the wall
	 * @param s1
	 *            the second space specifying the wall
	...
	public void placeWall(Space s0, Space s1) throws IllegalArgumentException {
		...
		SpaceNode nodeS0 = this.boardConfiguration
				.spaceAt(s0.getX(), s0.getY());
		SpaceNode nodeS1 = this.boardConfiguration
				.spaceAt(s1.getX(), s1.getY());

		if (temp.isHorizontal()) {
			SpaceNode nodeE0 = this.boardConfiguration.spaceAt(s0.getX(),
					s0.getY() + 1);
			SpaceNode nodeE1 = this.boardConfiguration.spaceAt(s1.getX(),
					s1.getY() + 1);
			...
			SpaceNode nodeE0 = this.boardConfiguration.spaceAt(s0.getX() + 1,
					s0.getY());
			SpaceNode nodeE1 = this.boardConfiguration.spaceAt(s1.getX() + 1,
					s1.getY());
	 ...
	 * 
	 * @param s0
	 *            The wall's first surrounding space.
	 * @param s1
	 *            The wall's second surrounding space.
	...
	public boolean canPlaceWall(Space s0, Space s1) {
		...
		if (wall.isHorizontal()) {
			...
			/*
			 * Space e0 = new Space(s0.getX(), s0.getY() + 1); Space e1 = new
			 * Space(s1.getX(), s1.getY() + 1); if (this.isWallHere(e0, s0)) {
			 * System.out.println("340"); return false; } if
			...
			/*
			 * Space e0 = new Space(s0.getX() + 1, s0.getY()); Space e1 = new
			 * Space(s1.getX() + 1, s1.getY()); if (this.isWallHere(e0, s0)) {
			 * System.out.println("351"); return false; } if
		...
		if (temp.isHorizontal()) {
			...
			SpaceNode nodeE1 = this.boardConfiguration.spaceAt(s1.getX(),
					s1.getY() + 1);
			/*
			 * Cutting the links from s0 to e0, and conversely s1 to e1, and
			 * conversely
			 * 
			 * e0 e1 X==========X s0 s1
			 */
	...
	public boolean isWallHere(Space startingSpace1, Space startingSpace2,
		 ...
		 * otherSideY = startingSpace2.getY(); if (startingSpace2.getX() -
		 * startingSpace1.getX() == 1) { if
		 * (boardConfiguration.spaceAt(startingX, startingY).getRightNode() ==
		 * null) return true; } else if (startingSpace2.getX() -
		 * startingSpace1.getX() == -1) { if
		 * (boardConfiguration.spaceAt(startingX, startingY).getLeftNode() ==
		 * null) return true; } else if (startingSpace2.getY() -
		 * startingSpace1.getY() == 1) { if
		 * (boardConfiguration.spaceAt(startingX, startingY).getTopNode() ==
		 * null) return true; } else if (startingSpace2.getY() -
		 * startingSpace1.getY() == -1) { if
	 ...
	 * 
	 * @param playerNumber
	 *            the player number in the current game.
	 * @throws IndexOutOfBoundsException
	 *             When the player number is larger or smaller than the total
	 *             number of players.
	 */
	 ...
	 * @param moveString
	 *            the Protocol String.
	 * @return A space object with the respective coordinates; -1 if a
	 ...
	 * @return the corresponding move string in the proper protocol form; X-X if
	...
	public String spaceToString(Space space) {
		...
		if ((space.getX() >= 0 && space.getX() <= 8)
				&& (space.getY() >= 0 && space.getY() <= 8)) {
