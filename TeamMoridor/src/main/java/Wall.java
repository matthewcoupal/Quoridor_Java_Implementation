...
/**
 * Logical Representation of a wall for placement evaluation.
 * 
...
public class Wall {
  ...
  public Wall(Space side0, Space side1) throws IllegalArgumentException {
    ...
    this.s1 = side1;
    /*
     * if (!isLegalWall()) throw new IllegalArgumentException("This isn't an actual wall"); this.s0
     * = side0; this.s1 = side1; // surroundingSpaces[0] = side0; // surroundingSpaces[1] = side1;
     */
  }
   ...
   */
  /*
   * public Space getWallSurroundingSpace(int index) { return surroundingSpaces[index]; }
   */
  ...
  public boolean isEqual(Wall otherWall) {
    /*
     * if ( (this.s0.getX() != otherWall.s0.getX()) || (this.s0.getY() != otherWall.s0.getY()) ||
     * (this.s1.getX() != otherWall.s1.getX()) || (this.s1.getY() != otherWall.s1.getY()) ) return
     * false; else return true;
     */
    if ((this.getS0().equals(otherWall.getS0()) && this.getS1().equals(otherWall.getS1()))
        || (this.getS0().equals(otherWall.getS1()) && this.getS1().equals(otherWall.getS0()))) {
      return true;
    }
    return false;
  }
  ...
  public boolean isHorizontal() {
    if ((s0.getY() != s1.getY()) || // y-coordinates must be equal
        ((s1.getX() - s0.getX() != 1) && // x-coordinate of the second
        // space below must be 1
        // greater than the
        // x-coordinate of the first
        // space below.
        (s1.getX() - s0.getX() != -1)) /*
                                        * || (s0.getY() == 8) || // out of bounds (s1.getY() == 8)
                                        * || // out of bounds (s0.getX() == 8) || // out of bounds
  ...
  public boolean isVertical() {
    if (((s0.getY() - s1.getY() != 1) && // y-coordinate of the first space
        // to the right must be 1
        // greater than the y-coordinate
        // of the second space to the
        // right.
        (s0.getY() - s1.getY() != -1))
        || (s1.getX() != s0.getX()) /*
                                     * || // x-coordinates must be equal (s0.getX() == 0) || // out
                                     * of bounds (s0.getY() == 0) || // out of bounds (s1.getX() ==
                                     * 0) || // out of bounds (s1.getY() == 8) // out of bounds
                                     */
    )
      return false;
    else
