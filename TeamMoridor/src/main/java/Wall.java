...
public class Wall {
	 ...
	 * 
	 * @param side0
	 *            the first end-point corresponding to the wall being
	 *            constructed.
	 * @param side1
	 *            the second end-point corresponding to the wall being
	 *            constructed.
	 * @throws IllegalArgumentException
	 *             if the end-points don't correspond to a valid wall.
	 */
	public Wall(Space side0, Space side1) throws IllegalArgumentException {
		this.s0 = side0;
		this.s1 = side1;
		/*
		 * if (!isLegalWall()) throw new
		 * IllegalArgumentException("This isn't an actual wall"); this.s0 =
		 * side0; this.s1 = side1; // surroundingSpaces[0] = side0; //
	 ...
	 */
	/*
	 * public Space getWallSurroundingSpace(int index) { return
	 * surroundingSpaces[index]; }
	 */

	/**
	 * Checks if a wall is equal to a given wall (i.e. is placed at the same
	 * location)
	 * 
	 * @param otherWall
	 *            The other wall
	...
	public boolean isEqual(Wall otherWall) {
		/*
		 * if ( (this.s0.getX() != otherWall.s0.getX()) || (this.s0.getY() !=
		 * otherWall.s0.getY()) || (this.s1.getX() != otherWall.s1.getX()) ||
		 * (this.s1.getY() != otherWall.s1.getY()) ) return false; else return
		 * true;
		 */
		if ((this.getS0().equals(otherWall.getS0()) && this.getS1().equals(
				otherWall.getS1()))
				|| (this.getS0().equals(otherWall.getS1()) && this.getS1()
						.equals(otherWall.getS0()))) {
			return true;
		}
		return false;
	}
	...
	public boolean isVertical() {
		if (((s0.getY() - s1.getY() != 1) && // y-coordinate of the first space
				...
				|| (s1.getX() != s0.getX()) /*
											 ...
											 * (s0.getX() == 0) || // out of
											 * bounds (s0.getY() == 0) || // out
											 * of bounds (s1.getX() == 0) || //
											 * out of bounds (s1.getY() == 8) //
											 * out of bounds
											 */
		)
			return false;
